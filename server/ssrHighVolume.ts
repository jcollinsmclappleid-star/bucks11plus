  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <ul>
        ...
        <li><strong>Print 11+ practice paper packs (GL Assessment format)</strong> — Established UK education publishers produce extensive GL Assessment-format practice packs covering all four domains: Verbal Reasoning, Non-Verbal Reasoning, Mathematics, and English Comprehension. Look for titles explicitly labelled "GL" or "GL Assessment style", with timed papers and (where available) score conversion guides.</li>
...
export function getScoreGuideHtml(): string {
  const url = `${BASE}/bucks-11-plus-how-scoring-works`;
  const title = "How the Bucks 11 Plus Score Works | Bucks 11 Plus Tests";
  const desc = "How the Bucks 11 Plus standardised score is calculated. What raw scores map to, how age standardisation works, and what score your child needs to qualify.";
  ...
  const faqs = [
    ...
    { question: "Can I calculate my child's exact 11+ score myself?", answer: "No. GL Assessment does not publish the raw-score-to-standardised-score conversion tables used for the actual Secondary Transfer Test, and the conversion changes year on year with paper difficulty. We deliberately do not provide a 'score calculator' because any such tool would produce a misleading number that is not the same as the official GL standardised score. The most reliable way to gauge your child's likely performance is through a GL Assessment-style readiness check that returns a practice score on the 121 scale — such as the free readiness check on Bucks 11 Plus Tests. This is an indicator only, not a substitute for the official GL standardised score." },
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>GL Assessment does not publish raw-score-to-standardised-score conversion tables for the Secondary Transfer Test. Some print practice paper publishers include approximate conversion guides alongside their own materials, but these are approximations and the actual conversion changes with each year's paper difficulty.</p>
      <p>The most reliable way to gauge your child's likely performance is through a validated readiness check. The free readiness check on Bucks 11 Plus Tests covers all four domains in GL Assessment question style and returns an instant practice score on the 121 scale, with a readiness band (Approaching, On Track, Secure). The practice score is an indicator only and is not a substitute for the official GL standardised score.</p>
    </section>
    ${ssrFaqSection(faqs)}
    <section class="ssr-related" style="margin-top:2rem;">
      <h2>Related</h2>
