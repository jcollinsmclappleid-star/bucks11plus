...
export function getPastPapersHtml(): string {
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>A free 12-question GL-style readiness check is available on this platform — it covers all four test domains and returns an instant standardised score estimate and readiness band benchmarked against the 121 qualifying threshold. This is a useful starting point before purchasing paper-based materials. It does not replace full practice papers, but it identifies which domains need the most preparation time.</p>
    </section>
    ${ssrFaqSection(faqs)}
    <section class="ssr-related" style="margin-top:2rem;">
      <h2>Related</h2>
...
export function getScoreGuideHtml(): string {
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>The most reliable way to estimate your child's likely standardised score is through a validated readiness check. The free readiness check on Bucks 11 Plus Tests covers all four domains in GL Assessment question style and returns an instant standardised score estimate benchmarked against the 121 threshold, with a readiness band (Approaching, On Track, Secure).</p>
    </section>
    ${ssrFaqSection(faqs)}
    <section class="ssr-related" style="margin-top:2rem;">
      <h2>Related</h2>
