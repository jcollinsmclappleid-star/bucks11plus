...
export function getPastPapersHtml(): string {
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <ul>
        <li><strong>Start with domain-specific practice</strong> (not full papers) — Identify and address weak question types in each domain before using timed papers as a whole. A diagnostic assessment at the start of Year 5 tells you exactly which areas need the most attention.</li>
      ...
      <p>A free 12-question GL-style diagnostic is available on this platform — it covers all four test domains and returns an instant standardised score estimate and readiness band benchmarked against the 121 qualifying threshold. This is a useful starting point before purchasing paper-based materials. It does not replace full practice papers, but it identifies which domains need the most preparation time.</p>
    </section>
    ${ssrFaqSection(faqs)}
    <section class="ssr-related" style="margin-top:2rem;">
      <h2>Related</h2>
...
export function getSampleQuestionsHtml(): string {
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>The free diagnostic on Bucks 11 Plus Tests includes 12 GL-style questions across all four domains — the fastest way to experience the actual question format and receive an instant score estimate. No account needed.</p>
    </section>
    ${ssrFaqSection(faqs)}
    <section class="ssr-related" style="margin-top:2rem;">
...
export function getScoreGuideHtml(): string {
  ...
  const desc = "How the Bucks 11 Plus standardised score is calculated. What raw scores map to, how age standardisation works, and what score your child needs to qualify. Free diagnostic to estimate your child's score.";
  ...
  const faqs = [
    ...
    { question: "Is there a Bucks 11 Plus score calculator I can use?", answer: "GL Assessment does not publish the raw-score-to-standardised-score conversion tables used for the actual Secondary Transfer Test, so it is not possible to calculate an exact predicted score from raw marks. The most reliable way to estimate your child's likely performance is through a GL Assessment-style diagnostic assessment that returns a standardised score estimate — such as the free diagnostic on Bucks 11 Plus Tests, which benchmarks performance across all four domains against the 121 threshold." },
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>The most reliable way to estimate your child's likely standardised score is through a validated diagnostic assessment. The free diagnostic on Bucks 11 Plus Tests covers all four domains in GL Assessment question style and returns an instant standardised score estimate benchmarked against the 121 threshold, with a readiness band (Approaching, On Track, Secure).</p>
    </section>
    ${ssrFaqSection(faqs)}
    <section class="ssr-related" style="margin-top:2rem;">
      <h2>Related</h2>
...
export function getTutorsGuideHtml(): string {
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>Tutoring can be highly effective when used well — but it is an input, not a guarantee. The most important factor in 11+ preparation is targeted, gap-focused practice. A tutor who identifies a child's specific weaknesses across the four test domains and addresses them systematically is valuable. A tutor who works through practice papers without diagnostic insight is less so.</p>
      <p>Many families prepare successfully without any tutoring, using a combination of a diagnostic assessment, targeted practice by domain, and timed full mock papers. Others use a digital platform alongside minimal tutoring to maximise cost-effectiveness. The right approach depends on your child's learning style, the availability of good tutors locally, and your budget.</p>
      ...
      <ul>
        ...
        <li><strong>Diagnostic approach</strong> — Good tutors begin with an assessment of where the child currently is, not by launching into a practice paper. They should be able to tell you within the first two sessions where the child's specific weaknesses are across all four domains.</li>
