...
export interface IStorage {
  ...
  updateUserTrial(userId: string, trialEndsAt: Date | null): Promise<User>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserTrial(userId: string, trialEndsAt: Date | null): Promise<User> {
    const [user] = await db.update(users)
      .set({ trialEndsAt })
      .where(eq(users.id, userId))
      .returning();
  ...
  async selectQuestionsForSession(userId: string, diagnosticId: string): Promise<Question[]> {
    ...
    if (diagnosticId === 'mini-1') {
      const FIXED_MINI_IDS = [
        ...
        '8868478d-f539-40a1-a4ca-a5c901abed1d', // NVR sequence easy (v6) — fill_cycle_only
