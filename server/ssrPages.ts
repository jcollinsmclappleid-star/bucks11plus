...
export function getTownHtml(slug: string): string | null {
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>No single preparation approach suits every child. Some children make rapid progress with independent digital practice and minimal parental involvement; others benefit from structured tutor-led sessions and feedback. What all effective preparation has in common: it is readiness-led (identifying specific gaps rather than repeating strong areas), it is progressive (building from domain skills to timed papers), and it includes realistic timed practice under audio conditions in the months before the test.</p>
...
export function getGrammarSchoolHtml(slug: string): string | null {
  ...
  const body = `
    ...
    <section class="ssr-section">
      ...
      <p>Practice with a GL Assessment-style readiness check early in Year 5 identifies which domains need the most work before the September Year 6 test. Domain-specific practice, timed papers, and realistic mock conditions in the summer holidays are the core of effective preparation.</p>
