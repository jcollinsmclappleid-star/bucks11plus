...
export function SeoPricingTeaser({ className = "", compact = false }: SeoPricingTeaserProps) {
  return (
    <div
      className={`rounded-2xl border-2 ${PRICING_UI.cardBorder} ${PRICING_UI.cardBg} p-5 md:p-6 ${PRICING_UI.cardShadow} ${className}`}
      data-testid="seo-pricing-teaser"
    >
      <div className="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-8">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <Sparkles className={`h-4 w-4 ${PRICING_UI.icon} shrink-0`} aria-hidden="true" />
            <p className={`text-xs font-bold uppercase tracking-[0.18em] ${PRICING_UI.eyebrow}`}>
            ...
            <ul className="mt-4 grid sm:grid-cols-2 gap-x-4 gap-y-2">
              {highlights.map((item) => (
                <li key={item} className="flex items-start gap-2 text-sm text-slate-700">
                  <CheckCircle2 className={`h-4 w-4 ${PRICING_UI.icon} shrink-0 mt-0.5`} aria-hidden="true" />
                  {item}
                </li>
              ))}
            </ul>
        ...
        <div className={`shrink-0 rounded-xl border ${PRICING_UI.innerCard} bg-white px-5 py-4 text-center shadow-sm lg:min-w-[230px]`}>
          ...
          <div className="mt-4 flex flex-col gap-2">
            ...
            <Button
              variant="outline"
              asChild
              className={`w-full font-semibold ${PRICING_UI.outlineBtn}`}
...
export function SeoPricingBar({ className = "" }: { className?: string }) {
  return (
    <div
      className={`rounded-xl border-2 ${PRICING_UI.cardBorder} ${PRICING_UI.barBg} px-4 py-4 md:px-5 md:py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${className}`}
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-2 shrink-0 w-full sm:w-auto">
        ...
        <Button
          ...
          size="sm"
          asChild
          className={`w-full sm:w-auto font-semibold ${PRICING_UI.outlineBtn}`}
          data-testid="button-seo-pricing-bar-free"
        >
